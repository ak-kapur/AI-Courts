...
# Function to search CaseMine
def search_casemine(query):
    search_url = f"https://www.casemine.com/search?q={query.replace(' ', '+')}"
    response = requests.get(search_url)
    soup = BeautifulSoup(response.content, "html.parser")

    results = []
    for result in soup.find_all('div', class_='casemine-case-title'):
        title = result.find('a').get_text()
        link = "https://www.casemine.com" + result.find('a')['href']
        ...
        if st.button("Get Search Results"):
            if question:
                with st.spinner("Searching..."):
                    results = search_casemine(question)
                    if results:
    ...
    if st.button("Search Similar Cases"):
        if pdf_files:
            with st.spinner("Generating case summary..."):
                ...
                with st.spinner("Searching..."):
                    results = search_casemine(summary_answer)
                    if results:
                        st.write(f"**Found {len(results)} similar cases:**")
                        for result in results:
                            st.write(f"[{result['title']}]({result['link']})")
